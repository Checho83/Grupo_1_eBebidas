...
function alreadyHaveEmail(email){
  if ((userController.usuarios.findIndex(usuario => usuario.email == email)) >= 0) 
    return true;
  else
    return false;
...
const validateRegister = [
  ...
  check('lastName')
    ...
    check('email').notEmpty().withMessage('Escribir un email').custom(email => {
      if (alreadyHaveEmail(email)) {
        throw new Error('Email ya registrado, indicar otro email')}
        return true;}).isEmail().withMessage('Formato de Email no valido'),
    ...
    check('pass').notEmpty().withMessage('Ingrese una contraseña adecuada')
                 .isLength({ min:3}).withMessage('Ingrese una contraseña de mas de 3 caracteres'),
